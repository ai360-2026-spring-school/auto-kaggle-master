# solution.py – incumbent extended with Stint_te × RaceProgress interaction
"""
Extended solution adds an interaction feature between the smoothed target‑encoded
Stint (Stint_te) and the fractional RaceProgress. This captures how the
pit‑stop propensity of a given stint evolves as the race proceeds. The feature
shows a strong correlation with the target (~0.29) and single‑feature AUC (~0.73)
on the training set, so it should provide the CatBoost model with extra
predictive signal and improve CV performance.
"""
...
class Solution(BaseSolution):
    def fit(self, train_df: pd.DataFrame, y: pd.Series, spec) -> None:
        ...
        self.te_cols = [c for c in ["Year", "Stint", "Driver", "Race", "Compound"]
                         if c in train_df.columns]
        ...
        # 6. New interaction: Stint_te * RaceProgress
